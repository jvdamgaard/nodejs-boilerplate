...
var jsCompressor = function(options, production) {
  ...
  // Get all manifest files in js folder. Read them. And return input as arrays

  function getManifests(done) {
    var manifests = {};

    // Find all _manifest.json files in js folder
    searchForFiles('_manifest.json', options.source, function(err, manifestFiles) {
      if (err) done(err);

      async.forEach(manifestFiles, function(manifestFile, next) {

        // Create new js type
        var dir = path.dirname(manifestFile);
        var type = _.last(dir.split(path.sep));
        manifests[type] = [];

        // Read manifest file and retrun value as array
  ...
  function watchFiles(manifests, done) {
    for (var type in manifests) {
      async.forEach(manifests[type], function(file, next) {
        console.log(file);
