...
var generateImage = function(files, callback) {
  ...
  mkdirp(path.dirname(retinaImage), function(err) {
    ...
    im.convert(commands, function(err, stdout, stderr) {
      ...
      log.succes('sprite', 'compiled', retinaImage);

      // Normal size (50%) sprite image
      im.convert([retinaImage, '-resize', '50%', nonRetinaImage], function(err, stdout, stderr) {
        if (err) return callback(err);
        if (stderr) return callback(stderr);

        log.succes('sprite', 'compiled', nonRetinaImage);

        callback(null, {retina: retinaImage, nonRetina: nonRetinaImage});
      });
    });
