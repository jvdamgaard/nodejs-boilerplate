...
var createJsFile = function(destination, sources, force, callback) {
...
var init = function(err, force) {
  ...
  getManifests(options.manifest, function(err, files) {
    ...
    for (var file in files) {
      ...
      createJsFile(file, files[file], force, function(err, jsFile, jsFiles) {
        if (err) return log.error('js', err);
        cache.add(jsFile);
