...
module.exportd.moveFile = function(fileFrom,fileTo,callback) {

    // TODO: async
    fs.readFile(fileFrom, function(err, data) {
      if (err) return callback(err);

      mkdirp(path.dirname(fileTo), function(err) {
        if (err) return callback(err);

        fs.writeFile(fileTo, data, function(err) {
          if (err) return callback(err, fileFrom, fileTo);
        });
      });
    });
  };