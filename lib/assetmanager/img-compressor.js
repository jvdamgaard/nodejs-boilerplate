...
module.exports = function(options, cache, production) {
  ...
  var compressImage = function(file,callback) {
    ...
    kraken.upload(file, function(status) {
      if (status.success) {
        fsExt.copyImageFromUrl(status.krakedURL, publicFile, function(err) {
          if (err) return log.error('img', err);
          log.succes('img', 'optimized', 'squeezed ' + Math.round(status.savedBytes/1000) +'kB (' + status.savedPercent + ') out of ' + publicFile);
        });
      } else {
