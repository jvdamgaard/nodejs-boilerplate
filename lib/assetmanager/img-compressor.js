...
module.exports = function(options, cache, production) {
  ...
  var compressImage = function(file,callback) {
    ...
    // Only squeeze images in production for saving "Limit on Kraken API"
    if (_.indexOf(krakenTypes, ext) === -1 || !options.KrakenAPI || !production) return callback(null, file);
    ...
    kraken.upload(file, function(status) {
      if (status.success) {
        fsExt.copyImageFromUrl(status.krakedURL, file, function(err) {
          ...
          log.succes('img', 'optimized', 'squeezed ' + Math.round(status.savedBytes/1000) +'kB (' + status.savedPercent + ') out of ' + file);
          callback(null, file);
        });
      } else {
