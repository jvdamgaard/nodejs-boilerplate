...
module.exports = function(options, cache, production) {
  ...
  var compressImage = function(file,callback) {
    ...
    //       if (err) return log.error('img', err);
    //       log.succes('img', 'optimized', 'squeezed ' + (Math.round(status.savedBytes/100)/10) +'kB (' + status.savedPercent + ') out of ' + file + ': total size: ' + (Math.round(status.krakedSize/100)/10) + 'kb');
    //       callback(null, file);
    //     });
    //   } else {
    ...
    console.log(file + ' should be sent to Kraken');
    callback(null, file);
